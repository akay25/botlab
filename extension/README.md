# botlab probe — a Chrome extension
...
The probe reads sixteen properties in both worlds and reports every field that
differs. A single divergence is direct evidence of a patch. This is the strongest
signal in the whole harness, and only an extension can measure it.
...
The badge on the icon shows the score for the active tab.
...
## Connect the probe to the harness

The extension cannot read the TLS handshake, so it scores five layers. The
server scores seven. Connect the two to get the full picture.
...
5. Browse. Each report reaches the harness and joins the session table.

The popup then shows two scores. The local score comes from five layers. The
harness score adds the TLS layer and the network layer.

Trust the harness certificate first. Open `https://127.0.0.1:8443/` in the same
browser and accept the warning. The extension cannot accept it for you.

## The layers that the extension measures

| Layer | Source |
|---|---|
| http | The real header order, read with `chrome.webRequest` |
| browser | The main world report |
| worlds | The difference between the main world and the isolated world |
| behavior | Pointer path, key timing, and the `isTrusted` flag |
| consistency | The User-Agent against every other claim |

The `behavior` layer holds one signal that the page harness cannot trust. A
content script reads `isTrusted` before page code can replace the event object.
An event with `isTrusted` set to false came from a script, not from a hand.

## Export the data

Press **Export history CSV** in the popup. The file holds one row per report
with the time, the label, the score, the earliest layer, and the divergent
fields. Use it as Table A of the protocol.

The harness export at `/export.csv` holds the same runs with the TLS columns
added. Join the two on the run label.
...
| `storage` | Hold the harness URL, the run label, and the history |
| `tabs` | Match a report to the tab that produced it |
...
3. The world comparison finds a patch that the control channel applied. It does
   ...
   changes the source patches both worlds at once.

The third limit is a finding, not a weakness. State it in the thesis. It marks
